...
const SSO_DESCRIPTION = `Initiates AWS SSO login for the specified environment, opening a browser window for the user to authenticate.
...
export interface SsoToolsRegistration {
  applyConfig(): void;
}

export function registerSsoTools(server: McpServer): SsoToolsRegistration {
  const ssoLogin = server.registerTool(
    "aws_sso_login",
    {
      title: "AWS SSO Login",
      description: SSO_DESCRIPTION,
      inputSchema: {
    ...
    () => ({
      content: [{ type: "text" as const, text: notConfiguredMessage() }],
    })
  );
          ...
          try {
            ...
            return {
              content: [
                {
                  type: "text",
                  text: `SSO login completed for profile "${profile}" (environment: ${env}). Client cache has been cleared.\n\nPlease tell the user: "Browser login was successful. You can now retry your request."`,
                },
              ],
            };
          } catch (error) {
